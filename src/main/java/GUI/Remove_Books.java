...
public class Remove_Books extends javax.swing.JFrame {
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        String books = txtID.getText();

        try (Session session = HibernateUtils.getSessionFactory().openSession()) {
            session.beginTransaction();

            CriteriaBuilder criteriaBuilder = session.getCriteriaBuilder();
            CriteriaDelete<Books> criteriaDelete = criteriaBuilder.createCriteriaDelete(Books.class);
            Root<Books> root = criteriaDelete.from(Books.class);
            Predicate predicate = criteriaBuilder.or(
                    criteriaBuilder.equal(root.get("bookId"), books),
                    criteriaBuilder.equal(root.get("namebook"), books)
            );
            ...
            if (rows > 0) {
                JOptionPane.showMessageDialog(this, "Book remove successfully");
            } else {
